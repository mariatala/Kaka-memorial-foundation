'use client';

import { useState } from 'react';
import { Inter, Gowun_Dodum } from 'next/font/google';
import { Plus, Pencil, Trash2, Check, X, BarChart2 } from 'lucide-react';

const inter = Inter({ weight: ['300', '400', '500', '600', '700'], subsets: ['latin'] });
const gowun = Gowun_Dodum({ weight: '400', subsets: ['latin'] });

export interface StatisticUI {
    id: number;
    prefix: string;
    value: string;
    suffix: string;
    description: string;
    order: number;
    createdAt: string;
    updatedAt: string;
}

type FormData = {
    prefix: string;
    value: string;
    suffix: string;
    description: string;
    order: string;
};

function emptyForm(): FormData {
    return { prefix: '', value: '', suffix: '', description: '', order: '0' };
}

function toFormData(s: StatisticUI): FormData {
    return {
        prefix: s.prefix,
        value: s.value,
        suffix: s.suffix,
        description: s.description,
        order: String(s.order),
    };
}

function validateForm(f: FormData): string | null {
    if (!f.description.trim()) return 'Description is required.';
    if (!f.value.trim()) return 'Value is required.';
    if (!/^\d+$/.test(f.value.trim())) return 'Value must be a whole number (digits only).';
    const orderNum = parseInt(f.order, 10);
    if (isNaN(orderNum)) return 'Order must be a number.';
    return null;
}

const fieldCls =
    'w-full py-2.5 px-1 border-b border-primary/30 bg-transparent outline-none text-primary text-sm placeholder:text-primary/30 focus:border-secondary transition-colors duration-200';

interface StatFormProps {
    data: FormData;
    onChange: (name: string, value: string) => void;
    onSubmit: () => void;
    onCancel: () => void;
    pending: boolean;
    error: string | null;
    submitLabel: string;
}

function StatForm({ data, onChange, onSubmit, onCancel, pending, error, submitLabel }: StatFormProps) {
    return (
        <div className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5">
                <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold uppercase tracking-wider text-primary/50">
                        Description <span className="text-red-500">*</span>
                    </label>
                    <input
                        className={fieldCls}
                        value={data.description}
                        onChange={e => onChange('description', e.target.value)}
                        placeholder="e.g. People Served by Kaka Foundation"
                    />
                </div>

                <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold uppercase tracking-wider text-primary/50">
                        Value <span className="text-red-500">*</span>{' '}
                        <span className="font-normal normal-case text-primary/30">(whole number)</span>
                    </label>
                    <input
                        className={fieldCls}
                        value={data.value}
                        onChange={e => onChange('value', e.target.value)}
                        placeholder="e.g. 500"
                        inputMode="numeric"
                    />
                </div>

                <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold uppercase tracking-wider text-primary/50">
                        Prefix{' '}
                        <span className="font-normal normal-case text-primary/30">(currency symbol or label before the number)</span>
                    </label>
                    <input
                        className={fieldCls}
                        value={data.prefix}
                        onChange={e => onChange('prefix', e.target.value)}
                        placeholder='e.g. $ or ₦ (leave blank for none)'
                    />
                </div>

                <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold uppercase tracking-wider text-primary/50">
                        Suffix{' '}
                        <span className="font-normal normal-case text-primary/30">(symbol after the number)</span>
                    </label>
                    <input
                        className={fieldCls}
                        value={data.suffix}
                        onChange={e => onChange('suffix', e.target.value)}
                        placeholder='e.g. + (leave blank for none)'
                    />
                </div>

                <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold uppercase tracking-wider text-primary/50">
                        Display Order{' '}
                        <span className="font-normal normal-case text-primary/30">(lower = appears first)</span>
                    </label>
                    <input
                        className={fieldCls}
                        value={data.order}
                        onChange={e => onChange('order', e.target.value)}
                        placeholder="0"
                        inputMode="numeric"
                    />
                </div>
            </div>

            {error && (
                <div className="px-4 py-3 bg-red-50 border-l-4 border-red-500 rounded-sm" role="alert">
                    <p className="text-red-700 text-sm">{error}</p>
                </div>
            )}

            <div className="flex items-center gap-3 pt-1">
                <button
                    onClick={onSubmit}
                    disabled={pending}
                    className="flex items-center gap-1.5 px-5 py-2 bg-secondary text-white text-sm font-semibold rounded-sm hover:bg-secondary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                    {pending
                        ? <span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                        : <Check size={14} />}
                    {submitLabel}
                </button>
                <button
                    onClick={onCancel}
                    disabled={pending}
                    className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-primary/60 hover:text-primary border border-primary/20 rounded-sm transition-colors"
                >
                    <X size={14} />
                    Cancel
                </button>
            </div>
        </div>
    );
}

function formatPreview(s: StatisticUI | FormData & { id?: number }): string {
    const value = 'value' in s ? s.value : '';
    const n = parseInt(value, 10);
    const formatted = !isNaN(n) && n >= 10000 ? n.toLocaleString() : value;
    return `${s.prefix}${formatted}${s.suffix}`;
}

export default function StatisticsEditor({ statistics: initial }: { statistics: StatisticUI[] }) {
    const [stats, setStats] = useState<StatisticUI[]>(initial);
    const [editingId, setEditingId] = useState<number | null>(null);
    const [editForm, setEditForm] = useState<FormData>(emptyForm());
    const [editError, setEditError] = useState<string | null>(null);
    const [isCreating, setIsCreating] = useState(false);
    const [createForm, setCreateForm] = useState<FormData>(emptyForm());
    const [createError, setCreateError] = useState<string | null>(null);
    const [pending, setPending] = useState(false);
    const [deleteConfirmId, setDeleteConfirmId] = useState<number | null>(null);
    const [deleteError, setDeleteError] = useState<string | null>(null);
    const [successId, setSuccessId] = useState<number | null>(null);

    function startEdit(stat: StatisticUI) {
        setEditingId(stat.id);
        setEditForm(toFormData(stat));
        setEditError(null);
        setIsCreating(false);
        setCreateError(null);
        setDeleteConfirmId(null);
    }

    function cancelEdit() {
        setEditingId(null);
        setEditError(null);
    }

    function startCreate() {
        const nextOrder = stats.length > 0 ? Math.max(...stats.map(s => s.order)) + 1 : 1;
        setIsCreating(true);
        setCreateForm({ ...emptyForm(), order: String(nextOrder) });
        setCreateError(null);
        setEditingId(null);
        setEditError(null);
        setDeleteConfirmId(null);
    }

    async function handleUpdate() {
        const err = validateForm(editForm);
        if (err) { setEditError(err); return; }
        setPending(true); setEditError(null);
        try {
            const res = await fetch(`/api/admin/statistics/${editingId}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    ...editForm,
                    order: parseInt(editForm.order, 10),
                }),
            });
            const data = await res.json();
            if (!res.ok) throw new Error(data.message);
            setStats(prev => prev.map(s => s.id === editingId ? data : s));
            setEditingId(null);
            setSuccessId(data.id);
            setTimeout(() => setSuccessId(null), 2500);
        } catch (err) {
            setEditError(err instanceof Error ? err.message : 'Update failed.');
        } finally {
            setPending(false);
        }
    }

    async function handleCreate() {
        const err = validateForm(createForm);
        if (err) { setCreateError(err); return; }
        setPending(true); setCreateError(null);
        try {
            const res = await fetch('/api/admin/statistics', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    ...createForm,
                    order: parseInt(createForm.order, 10),
                }),
            });
            const data = await res.json();
            if (!res.ok) throw new Error(data.message);
            setStats(prev => [...prev, data].sort((a, b) => a.order - b.order));
            setCreateForm(emptyForm());
            setIsCreating(false);
            setSuccessId(data.id);
            setTimeout(() => setSuccessId(null), 2500);
        } catch (err) {
            setCreateError(err instanceof Error ? err.message : 'Create failed.');
        } finally {
            setPending(false);
        }
    }

    async function handleDelete(id: number) {
        setPending(true); setDeleteError(null);
        try {
            const res = await fetch(`/api/admin/statistics/${id}`, { method: 'DELETE' });
            if (!res.ok) { const d = await res.json(); throw new Error(d.message); }
            setStats(prev => prev.filter(s => s.id !== id));
            setDeleteConfirmId(null);
            if (editingId === id) setEditingId(null);
        } catch (err) {
            setDeleteError(err instanceof Error ? err.message : 'Delete failed.');
        } finally {
            setPending(false);
        }
    }

    return (
        <div className={inter.className}>
            {/* Header row */}
            <div className="flex items-center justify-between mb-6">
                <div className="space-y-1">
                    <h2 className={`text-xl font-semibold text-primary ${gowun.className}`}>Statistics</h2>
                    <div className="w-10 h-0.5 bg-secondary rounded-full" />
                </div>
                {!isCreating && (
                    <button
                        onClick={startCreate}
                        className="flex items-center gap-1.5 px-4 py-2 bg-primary text-white text-sm font-semibold rounded-sm hover:bg-primary/90 transition-colors"
                    >
                        <Plus size={15} />
                        New Stat
                    </button>
                )}
            </div>

            {deleteError && (
                <div className="mb-4 px-4 py-3 bg-red-50 border-l-4 border-red-500 rounded-sm flex items-center justify-between">
                    <p className="text-red-700 text-sm">{deleteError}</p>
                    <button onClick={() => setDeleteError(null)} className="text-red-400 hover:text-red-600">
                        <X size={14} />
                    </button>
                </div>
            )}

            {/* Create form */}
            {isCreating && (
                <div className="bg-white rounded-lg border border-secondary/30 px-6 py-5 mb-6 shadow-sm">
                    <p className={`text-sm font-semibold text-primary mb-5 ${gowun.className}`}>New Statistic</p>
                    <StatForm
                        data={createForm}
                        onChange={(name, value) => setCreateForm(prev => ({ ...prev, [name]: value }))}
                        onSubmit={handleCreate}
                        onCancel={() => { setIsCreating(false); setCreateError(null); }}
                        pending={pending}
                        error={createError}
                        submitLabel="Add Statistic"
                    />
                </div>
            )}

            {/* Stats list */}
            {stats.length === 0 ? (
                <div className="bg-white rounded-lg border border-primary/10 py-16 text-center">
                    <BarChart2 size={32} className="mx-auto text-primary/20 mb-3" />
                    <p className="text-primary/40 text-sm">No statistics yet. Add one above.</p>
                </div>
            ) : (
                <div className="space-y-4">
                    {stats.map(stat => (
                        <div
                            key={stat.id}
                            className={`bg-white rounded-lg border overflow-hidden transition-colors duration-500 ${
                                successId === stat.id ? 'border-secondary/50 shadow-sm' : 'border-primary/10'
                            }`}
                        >
                            {/* Card row */}
                            <div className="flex items-center gap-4 px-6 py-5">
                                {/* Visual accent */}
                                <div className="w-12 h-12 rounded-lg bg-secondary/10 flex items-center justify-center shrink-0">
                                    <span className="text-secondary text-sm font-bold tabular-nums">
                                        {formatPreview(stat)}
                                    </span>
                                </div>

                                {/* Info */}
                                <div className="flex-1 min-w-0">
                                    <p className="font-semibold text-primary">{stat.description}</p>
                                    <p className="text-xs text-primary/40 mt-0.5">
                                        Order: {stat.order}
                                        {successId === stat.id && (
                                            <span className="ml-2 text-secondary font-medium">✓ Saved</span>
                                        )}
                                    </p>
                                </div>

                                {/* Actions */}
                                {editingId !== stat.id && (
                                    <div className="flex items-center gap-2 shrink-0">
                                        <button
                                            onClick={() => startEdit(stat)}
                                            className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-primary/60 hover:text-secondary border border-primary/20 hover:border-secondary/40 rounded-sm transition-colors"
                                        >
                                            <Pencil size={12} />
                                            Edit
                                        </button>

                                        {deleteConfirmId === stat.id ? (
                                            <div className="flex items-center gap-1">
                                                <button
                                                    onClick={() => handleDelete(stat.id)}
                                                    disabled={pending}
                                                    className="px-3 py-1.5 text-xs font-semibold bg-red-500 text-white rounded-sm hover:bg-red-600 disabled:opacity-50 transition-colors"
                                                >
                                                    Confirm
                                                </button>
                                                <button
                                                    onClick={() => setDeleteConfirmId(null)}
                                                    className="px-3 py-1.5 text-xs font-medium text-primary/60 border border-primary/20 rounded-sm"
                                                >
                                                    Cancel
                                                </button>
                                            </div>
                                        ) : (
                                            <button
                                                onClick={() => { setDeleteConfirmId(stat.id); setDeleteError(null); }}
                                                className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-red-400 hover:text-red-600 border border-red-200 hover:border-red-400 rounded-sm transition-colors"
                                            >
                                                <Trash2 size={12} />
                                                Delete
                                            </button>
                                        )}
                                    </div>
                                )}
                            </div>

                            {/* Inline edit form */}
                            {editingId === stat.id && (
                                <div className="border-t border-primary/10 px-6 py-5 bg-primary/[0.02]">
                                    <p className={`text-sm font-semibold text-primary mb-5 ${gowun.className}`}>
                                        Editing: {stat.description}
                                    </p>
                                    <StatForm
                                        data={editForm}
                                        onChange={(name, value) => setEditForm(prev => ({ ...prev, [name]: value }))}
                                        onSubmit={handleUpdate}
                                        onCancel={cancelEdit}
                                        pending={pending}
                                        error={editError}
                                        submitLabel="Save Changes"
                                    />
                                </div>
                            )}
                        </div>
                    ))}
                </div>
            )}
        </div>
    );
}
