...
export default async function AdminMembershipsPage() {
    ...
    if (!session?.user || session.user.role !== 'admin') {
        redirect('/sign-in?callbackUrl=/admin/memberships');
    }
    ...
    return (
        <div className={`min-h-screen bg-light ${inter.className}`}>
            ...
            <div className="w-full bg-primary pt-24 pb-16 px-6 md:px-16">
                <div className="max-w-5xl mx-auto">
                    ...
                    <p className="text-accent-three text-sm mt-3">
                        {memberships.length} subscriber{memberships.length !== 1 ? 's' : ''} — use &ldquo;Copy all&rdquo; to export emails for a campaign
                    </p>
                </div>
            </div>
            ...
            <div className="max-w-5xl mx-auto px-6 md:px-16 py-12">
                <MembershipsTable memberships={serialized} />
            </div>
