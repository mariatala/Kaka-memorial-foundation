...
export default function DonateSection() {
	...
	const effectiveAmount =
		amount === 'Other' ? customAmount : (amount as number);
			...
			<div className="w-full lg:w-2/3 px-8 md:px-12 py-10 space-y-6">
				...
				<div className="w-full max-w-md space-y-4">
					<button
						...
						className="w-full bg-secondary hover:bg-secondary-dark disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-3 rounded-sm transition-colors duration-300"
					>
						Donate{' '}
						{effectiveAmount > 0 ? `$${effectiveAmount}` : 'Now'}{' '}
						{frequency}
					</button>
					<PayPalPayment amount={effectiveAmount > 0 ? effectiveAmount : 10} />
				</div>
			</div>
